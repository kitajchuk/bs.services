...
const getSite = function ( req ) {
    return new Promise(( resolve, reject ) => {
        prismic.api( core.config.api.access, apiOptions ).then(( api ) => {
            const form = api.form( "site" ).pageSize( 100 ).ref( getRef( req, api ) );
            ...
            form.submit().then(( json ) => {
                const docs = {
                    ...
                    casestudy: json.results.filter(( doc ) => {
                        return (doc.type === "casestudy");
                    }),
                    page: json.results.filter(( doc ) => {
                ...
                for ( let i in docs.site.data ) {
                    ...
                    if ( i !== "navi" ) {
                        ...
                        // Handle `string` values...
                            ...
                            site.data[ i ] = docs.site.data[ i ];
                        }
                    }
                }

                // Normalize navi context
                docs.site.data.navi.forEach(( slice ) => {
...
const getDataForPage = function ( req, listener ) {
    return new Promise(( resolve, reject ) => {
        ...
        // Here we skip querying since we load in getSite()
        } else if ( cache.docs[ req.params.type ] ) {
            data.items = cache.docs[ req.params.type ];
            data.item = req.params.uid ? getDoc( req.params.uid, data.items ) : undefined;
